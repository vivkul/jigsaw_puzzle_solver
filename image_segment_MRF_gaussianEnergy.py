...
def get_energy_quad(i,j,orientation,im,gaussian_array):
...
def pre_compute_energy(im):
	...
	w,h=im.size
	l=max(w/2,h/2)
	gaussian_array=np.empty([l])
	for i in range(l):
		gaussian_array[i]=norm.pdf(i)
	for i in range(4):
		for j in range(i+1,4):
			Map[0,i,j]=get_energy_quad(i,j,0,im,gaussian_array)		#When i is on left of j
			Map[0,j,i]=get_energy_quad(j,i,0,im,gaussian_array)		#When j is on left of i
			Map[1,i,j]=get_energy_quad(i,j,1,im,gaussian_array)		#When i is on top of j
			Map[1,j,i]=get_energy_quad(j,i,1,im,gaussian_array)		#When j is on top of i
